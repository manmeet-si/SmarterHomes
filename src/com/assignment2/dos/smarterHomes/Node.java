...
public class Node extends UnicastRemoteObject implements ElectionNode {
	...
	// Range for the period to check if heard from leader, [1,7]
	...
	public String getLeader(HashSet<String> connectionsList) {
		int min = 0;
		int max = connectionsList.size();
		messagePeriod = (min + (int) (Math.random() * ((max - min) + 1))) * 1000;
		int id = min + (int) (Math.random() * (max - min));
		java.util.Iterator<String> it = connectionsList.iterator();
		int c = 0;
		String leader = "GatewayManager";
		while(c < id) {
			leader = it.next();
			c++;
		}
